...
void lcd_init()
{
	...
	temp1=0x28;		/* function set with DL=4 bits, N=2lines , F=5*8 dots */
	lcd_com();
	delay_lcd(30000);
	
	temp1=0x0c;		/* Display on with cursor and blinking off */
	lcd_com();
	delay_lcd(30000);

	temp1=0x06;		//Entry set mode
	lcd_com();
	delay_lcd(800);
	...
	temp1=0x80;		//Set data register address
	lcd_com();
	delay_lcd(10000);
	return;
}
